...
public class ResolverController {
     ...
     *   접미사(suffix): .html
     *   핸들러 메소드가 반환하는 문자열은 뷰리졸버로 향하게하고 'redirect:'가 들어간 문자열의 경우에는 접두사와
     *   접미사가 추가되지 않는다.
     * */
    ...
    public String stringRedirect(Model model) {
        ...
        /* 설명. redirect 되어 요청이 왔을 때 응답되는 view의 재료가 된다(지금은 model에 담긴 값이 전달되지 않음)
        // (main.html의 재료가 되는 것) */
        model.addAttribute("message1", "문자열로 뷰 이름 반환하여 리다이렉트");

        // 302번 요청을 하며 main.redirect가 열림?
...
//
//            /* 설명. 스프링부트에서 리다이렉트시에 값이 전달되게 하고 싶다면 RedirectAttributes 활용 */
...
//    }
    ...
    public ModelAndView modelAndViewRedirectFlashAttribute(ModelAndView mv,
        ...
        rttr.addFlashAttribute("flashMessage2",
                "ModelAndView를 이용한 redirect arttr");
        mv.setViewName("redirect:/");
        return mv;
    }
}
